  ...
  walkDir(dir).then(files => {
    for (let i = 0; i < files.length; i++) {
      if (typeof files[i] === 'object') {
        // Directory!
        const dirMetadata = getFileMetadata(files[i].dir).then(metadata => {console.log(metadata)});
        for (let j = 0; j < files[i].files.length; j++) {
          const fileMetadata = getFileMetadata(files[i].files[j]).then(metadata => {console.log(metadata)});
        }
      } else {
        // File!
        const fileMetadata = getFileMetadata(files[i]).then(metadata => {console.log(metadata)});
      }
    }
  });
}

...
function ExpandDir(dirPath) {
  if (typeof dirPath === 'object') {
    ...
    getFileMetadata(dirPath[i].dir).then(metadata => {console.log(metadata)});
    for (let j = 0; j < dirPath[i].files.length; j++) {
      if (typeof dirPath[i] === 'object') {
        ExpandDir(dirPath[i].files[j]);
      } else {
        getFileMetadata(dirPath[i].files[j]).then(metadata => {console.log(metadata)});
      }
    }
  } else {
    // File!
    console.log("File!")
    getFileMetadata(dirPath[i]).then(metadata => {console.log(metadata)});
  }
}

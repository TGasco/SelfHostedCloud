...
function ExpandDir(dirPath) {
  if (typeof dirPath === 'object') {
    // Directory!
    getFileMetadata(dirPath.dir).then(metadata => {console.log(metadata)});
    for (let j = 0; j < dirPath.files.length; j++) {
      if (typeof dirPath === 'object') {
        ExpandDir(dirPath.files[j]);
      } else {
        for (let x = 0; x < dirPath.files[j].files.length; x++) {
          getFileMetadata(dirPath.files[j].files[x]).then(metadata => {console.log(metadata)});
        }
      }
    }
  } else {
    // File!
    console.log("File!")
    getFileMetadata(dirPath).then(metadata => {console.log(metadata)});
  }
}

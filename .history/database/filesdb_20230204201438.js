...
function ExpandDir(dirPath) {
  ...
  if (typeof currPath === 'object') {
    getFileMetadata(currPath.dir).then(metadata => {console.log(metadata)});
    for (let j = 0; j < currPath.files.length; j++) {
      currPath = ExpandDir(currPath[j]);
    }
  } else {
    for (let x = 0; x < currPath.length; x++) {
      getFileMetadata(currPath.files[x]).then(metadata => {console.log(metadata)});
    }
  }
}

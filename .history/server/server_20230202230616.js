...
async function getFileMetadata(filePath, callback) {
  ...
  const data = _stat(filePath, function(err, stats) {
    //Checking for errors
    if(err){
        console.log(err)
    }
...
var file = await getFileMetadata(filePath).then(console.log(file));
// db(file)
// get_req('login.html')
